...
int main(int argc, char **argv){
	...
	double min_cone_ang = 0.97; //14 deg -> 0.24 Rad
	double back_ang = -0.4; // 113 deg -> 1.98 Rad
	back_ang = acos(back_ang);
	double max_phi = acos(min_cone_ang);
	double back_dist = 1.;
	double dist_unclustered = 10.;
	double cut_ucp_ratio = 0.20;
	double cut_bottom_dist = 10.;

	//double eps_ang = atof(argv[3]);
	double eps_ang = 0.314159;
	double ephi_print = eps_ang*(100.);
	//double eps_dist = atof(argv[4]);
	double eps_dist = 1.5;
	double er_print = eps_dist*(100.);
	...
	int edist_count = 10;
	int eang_count = 10;
	double min_eang = -1;//~45 deg
	min_eang = acos(min_eang);
	double max_edist = 5.;
	...
	for (int i = 0; i <= eang_count; ++i){
		EANG.push_back(i * (min_eang/eang_count));
	//	EANG.push_back(0.24);
	}
	*/
	///////////////////
	//Define root output file
		TFile *f_output;
	if(track_num == -1){
			f_output = TFile::Open(Form("Cl_ephi_%d_er_%d_alpha_%d.root",(int)ephi_print,(int)er_print,(int)al_print),"RECREATE");
	...
	TNtuple *nt_results = new TNtuple("nt_results","nt_results","C_ord:alpha:M_ord:Cone_Cosphi:max_phi:ord_pca_pts:sel:M_sel:evr:min_Costheta:max_theta:purity:efficiency:ver_rms:ver_mean");
	...
	///////////////////
	//READ IN MICHEL LIST
	ifstream csv_infile("Michel_candidates_vertex_v8.csv");
	...
	small_tracks = 0;
	total_num_tracks = 0;
	tracks_survived_ord_alg = 0;
	michels_survived_ord_alg = 0;
	michel_count = 0;
	track_selected_as_michel = 0;
	eigenval_cut = 0;
	ord_alg_cutouts = 0;
	ord_alg_cutouts_michels = 0;
	sample_counter = 0;
	pca_count = 0;
	...
	std::vector<double> v_vertex_res;
	...
	while(std::getline(ifs, line)){
		...
		for (int c = 0; c < num_clusters; ++c){
			...
			bool closest_point_not_found = false;
			...
			//if(is_michel && far_ucp_ratio > 0.0) cout << far_ucp_ratio << endl;

			if (far_ucp_ratio > cut_ucp_ratio){
				if(is_michel){
		//			cout << far_ucp_ratio << endl; 
			...
			for(int i = 0; i < ord_trk.size(); ++i){
	    		nt_trk_pts_ord->Fill(run_num,ev_num,cluster,is_ordered,ord_trk.at(i).x,ord_trk.at(i).y,ord_trk.at(i).z,true);
	    	}
	    	for(int i = 0; i < points_gd.size(); ++i){
	    		nt_trk_pts_ord->Fill(run_num,ev_num,cluster,is_ordered,points_gd.at(i).x,points_gd.at(i).y,points_gd.at(i).z,false);
	    	}
	...
	cout << "########################## Results ################################" << endl;
	cout << "###################################################################" << endl;
	cout << "Purity = " << purity << ", Efficiency = " << efficiency << ", Vertex RMS = " << ver_rms << ", Vertex Mean = " << ver_mean << ", Vertex sigma = " << ver_sd << endl;
	...
	nt_results->Fill(tracks_survived_ord_alg,alpha,michels_survived_ord_alg,min_cone_ang,max_phi,ang_points,track_selected_as_michel,michel_count,EVR,min_costheta,theta,purity,efficiency,ver_rms,ver_mean);	
	f_output->Write();
	f_output->Close();
	cout << "DONE" << endl;
	return 0;
